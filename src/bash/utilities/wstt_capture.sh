...
# Input Capture type
while true; do
    print_prompt "Capture Type: [1] Full (all channels) [2] Filtered (by BSSID) [3] Filtered (by Channel): "
    read -r CAP_TYPE

    if [[ "$CAP_TYPE" == "1" || "$CAP_TYPE" == "2" || "$CAP_TYPE" == "3" ]]; then
        break
    else
        print_fail "Invalid selection. Please enter 1, 2, or 3"
    fi
done
...
# Filtered by Channel
    ...
    sudo iw dev "$INTERFACE" set channel "$CHANNEL"
    CURRENT_CHANNEL=$(iw dev "$INTERFACE" info | awk '/channel/ {print $2}')
    print_info "Capture filter: Channel=$CURRENT_CHANNEL"
    print_blank
    ...
    if [ "$CAP_MODE" = "1" ]; then
        while true; do
            ...
            read -r DURATION
            DURATION="${DURATION:-$DEFAULT_FILTERED_CAPTURE_DURATION}"
            if [[ "$DURATION" =~ ^[0-9]+$ ]]; then break; else print_fail "Invalid input. Enter a numeric value (seconds)"; fi
        done
        ...
        print_action "Starting capture"
        sudo timeout "$DURATION" tcpdump -i "$INTERFACE" -w "$OUTPUT_FILE"
        OUTPUT_PARAMS="Channel=$CURRENT_CHANNEL | Duration=$DURATION seconds"
        ...
        while true; do
            ...
            read -r MAX_PACKETS
            MAX_PACKETS="${MAX_PACKETS:-$DEFAULT_FILTERED_CAPTURE_PACKETS}"
            if [[ "$MAX_PACKETS" =~ ^[0-9]+$ ]]; then break; else print_fail "Invalid input. Enter a numeric value"; fi
        done
        ...
        print_action "Starting capture"
        sudo tcpdump -i "$INTERFACE" -c "$MAX_PACKETS" -w "$OUTPUT_FILE"
        OUTPUT_PARAMS="Channel=$CURRENT_CHANNEL | Packets=$MAX_PACKETS"
    fi

else
    print_fail "Invalid Capture type. Please enter 1, 2, or 3."
fi
